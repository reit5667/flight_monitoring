...
## Сессия 2026-06-19 — Рефакторинг + реальный тест (Вьетнам)

### Почему убрали Agoda

Agoda изменил URL-формат `/flights/search` → возвращает 302 на `/pagenotfound.html`. Починка потребовала бы реверс-инжиниринга нового URL. При этом Agoda функционально дублирует Trip.com (оба — агрегаторы для Азии). Решение: убрать, оставить Aviasales + Trip.com.

### Trip.com WAF — почему оставили как есть

Trip.com возвращает HTTP 432 (нестандартный код WAF) на headless Playwright, даже с реалистичным UA. Решения: playwright-stealth (не установлен), residential proxy (нет инфраструктуры). Код уже падает gracefully — connector возвращает `[]`, pipeline фиксирует в `errors` и продолжает. Для портфолио достаточно; для продакшена нужен прокси.

### `db.py` — зачем вынесли `get_conn()`

`_get_conn()` был продублирован в 5 модулях: `warehouse/current.py`, `scheduler/pipeline.py`, `scheduler/flow.py`, `planner/search_planner.py`, `storage/raw.py`. Это классический copy-paste — если меняется DSN или параметры подключения, нужно обновлять в 5 местах. Вынесли в `db.py` с публичным именем `get_conn`. Внутри модулей алиасируем как `from db import get_conn as _get_conn` чтобы не трогать остальной код.

### Notifications не были подключены к pipeline

`notifications/rules.py`, `dedup.py`, `telegram.py` были написаны (TASK-023–025) но ни разу не вызывались. `pipeline.py` не делал ни одного import из `notifications/`. Исправлено: в блоке `if events:` перед warehouse-записями открывается соединение, для каждого события вызывается `check_notification_rules(event, route, conn)` → `should_send` → `send_notification`. Важно именно ДО warehouse write: history должна отражать предыдущее состояние, чтобы HISTORICAL_MIN сработал корректно.

### Баг в test_warehouse_events: cleanup только после теста

`@pytest.fixture(autouse=True)` с `yield` в начале очищает таблицу только после теста. После нескольких прогонов пайплайна в БД накапливались события, и `assert COUNT(*) == 3` падал (находил 13). Исправлено: добавили DELETE до `yield` (pre-test setup), чтобы тест всегда стартовал с чистой таблицей.

### Реальный тест: Москва/Сочи/Краснодар → Вьетнам, август 2026

Добавили 12 маршрутов в БД (6 туда + 6 обратно), прогнали pipeline через Travelpayouts API. Результат:

**Лучший вариант туда-обратно на двоих:**
- Москва → Ханой **04.08** (Hainan, 1 пересадка) + Ханой → Москва **17.08** (Air China, 1 пересадка)
- ~93 000 руб на двоих (~46 500 руб/чел)
- Покупать как **два отдельных билета в один конец**, не туда-обратно — Aviasales при поиске round-trip показывал 109 000 руб (другая комбинация рейсов)

**Почему Travelpayouts-цены расходятся с Aviasales:**
API возвращает кэш за 48ч за 1 взрослого. На 2 взрослых + реальная доступность мест = другие цены. Использовать API как индикатор направления поиска, не как гарантию финальной цены.
