...
| 2026-06-27 | TASK-037 | Автодополнение городов: search_cities() по частичному вводу; двухшаговый ввод (сначала отправление, потом назначение); кнопки-подсказки при нераспознанном тексте; cb_city_pick хендлер |
| 2026-06-27 | TASK-038 | Telegram Mini App: dashboard/server.py (stdlib HTTP, /api/history + /api/routes); dashboard/miniapp.html (Chart.js, адаптация к теме Telegram); кнопка «📊 График цен» в результатах поиска (показывается только если MINIAPP_URL задан) |
